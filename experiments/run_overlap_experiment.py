...
- For each query segment, retain a contiguous overlap_ratio fraction
    of its points.
...
def truncate_segment(
    ...
    """
    Retain a contiguous ratio-sized portion of a segment.

    The random start models entering or leaving a place partway through
    the observation window while preserving the local curve geometry.
...
def run_overlap_experiment():
    ...
    for ratio in overlap_ratios:
        ...
        seg7_dtw = [segments_query_sub[i] for i in indices]
        seg3_dtw = list(segments_ref)
        ...
        rows.append({
            ...
            "f1": metrics_dtw["f1"],
            "num_query_segments": len(indices),
            "num_reference_segments": len(seg3_dtw),
