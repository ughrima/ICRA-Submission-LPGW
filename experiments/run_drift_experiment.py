...
Experiment:
    For each drift_std_per_step in [0.0, 0.1, 0.2, 0.5, 1.0] meters:
        ...
        4. Add integrated random-walk drift to the QUERY trajectory.
...
This isolates the effect of smooth, accumulating odometry drift on the
detector rather than allowing the perturbation to change the labels.

The same standardized random walk is scaled by sigma:
    sigma = 0.1 -> 0.1 * base_steps
    sigma = 0.2 -> 0.2 * base_steps
    ...
...
def zero_center_segments(segments):
    """
    Subtract the spatial mean from each segment.

    This isolates local shape geometry from global trajectory shift.
    """
    return [segment - segment.mean(axis=0) for segment in segments]
...
def run_drift_experiment():
    ...
    # -----------------------------------------------------------------
    # Create ONE standardized integrated random-walk realization.
    #
    ...
    base_steps = rng.normal(
        loc=0.0,
        scale=1.0,
        size=query_xyz_clean.shape,
    )
    base_drift = np.cumsum(base_steps, axis=0)
    ...
    # Fix the LPGW normalization scale from the clean reference database.
    # The same scale must be used at every drift level so the ablation
    # measures drift rather than a changing normalization baseline.
    clean_diameters = [
        float(np.max(cdist(segment, segment)))
        for segment in reference_segments_clean
    ]
    fixed_global_scale = float(np.median(clean_diameters))
    print(
        f"Fixed clean reference global scale: "
        f"{fixed_global_scale:.3f} m"
    )
    ...
    # -----------------------------------------------------------------
    # Integrated-drift sweep
    # -----------------------------------------------------------------

    for sigma in drift_stds_per_step:

        print()
        print("=" * 70)
        print(
            f"DRIFT STD PER STEP = {sigma:.2f} m"
        )
        print("=" * 70)

        # -------------------------------------------------------------
        # Add integrated drift ONLY to the query input.
        # -------------------------------------------------------------
        ...
        query_segments_noisy_centered = zero_center_segments(
            query_segments_noisy
        )
        reference_segments_clean_centered = zero_center_segments(
            reference_segments_clean
        )

        D_lpgw = detector.compute_distance_matrix(
            query_segments_noisy_centered,
            reference_segments_clean_centered,
        )
        ...
        rows.append(
            {
                "drift_std_per_step_m": sigma,
                "method": "LPGW",
                "precision": metrics_lpgw["precision"],
                "recall": metrics_lpgw["recall"],
                "f1": metrics_lpgw["f1"],
                "num_query_segments": min_len_clean,
                "num_reference_segments": len(reference_segments_clean),
        ...
        seg3_sub = list(reference_segments_clean)
        ...
        seg7_sub_centered = zero_center_segments(seg7_sub)
        seg3_sub_centered = zero_center_segments(seg3_sub)
        ...
        rows.append(
            {
                ...
                "f1": metrics_dtw["f1"],
                "num_query_segments": len(baseline_indices),
                "num_reference_segments": len(seg3_sub),
