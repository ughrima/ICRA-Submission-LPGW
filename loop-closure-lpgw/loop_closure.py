...
class LoopClosureDetector:
    ...
    def select_reference(
        ...
        if strategy != "robust":
            raise ValueError(
                "strategy must be one of: robust, first, middle, last"
            )
        ...
        for seg in segments:
            ...
            complexity.append(float(np.var(D)))

            bbox = np.ptp(seg, axis=0)

            # Avoid unstable 3-D volume estimates when a dimension is
            # nearly zero.
            volume = float(
                np.prod(np.maximum(bbox, 1e-8))
            )

            density.append(len(seg) / volume)

        complexity = np.asarray(complexity)
        density = np.asarray(density)

        complexity /= max(complexity.max(), 1e-12)
        density /= max(density.max(), 1e-12)

        size_score = (
            1.0
            - np.abs(sizes - median_size)
            / max(median_size, 1e-12)
        )

        # Same heuristic as your previous implementation, but kept
        # outside the mathematical LPGW core.
        score = (
            0.4 * size_score
            + 0.3 * complexity
            + 0.3 * density
